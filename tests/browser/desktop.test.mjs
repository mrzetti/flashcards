...
test('Explorer renders the catalog, filters by search and shows placeholder thumbnails', async () => {
  ...
  try {
    ...
    assert.equal(await page.locator('.card-tile').count(), 11);
    assert.equal(await page.locator('.task-pane, [data-filter]').count(), 0);
    assert.match(await page.locator('#status-count').textContent(), /11 cards/);
    ...
    assert.match(await page.locator('#explorer .explorer-status').textContent(), /Showing 1 of 11 cards/);
